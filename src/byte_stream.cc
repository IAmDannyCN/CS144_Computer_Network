...
ByteStream::ByteStream( uint64_t capacity ) :
			capacity_( capacity ),
			buffer_(), buffer_view_(),
			closed_(false), curlen_(0), pushcnt_(0), popcnt_(0) {
				#ifdef BYTE_DEBUG
...
string_view Reader::peek() const
{
	...
	#ifdef BYTE_DEBUG
	cout << ">>>>"; for(long unsigned int i = 0; i < buffer_.size(); i++) cout << "| " << _Out(buffer_[i]); cout << endl;
	cout << "++++"; for(long unsigned int i = 0; i < buffer_view_.size(); i++) cout << "| "  << _Out(buffer_view_[i]); cout << endl;
	#endif
