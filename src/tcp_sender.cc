...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  while ( modified_window_size_ > n_outstanding_ ) {
...
#ifdef TCPSENDER_DEBUG
    cout << "--------" << endl;
    cout << "window_size = " << window_size_ << "(" << modified_window_size_ << ")"
         << ", n_outstanding = " << n_outstanding_ << endl;
#endif
    ...
    // uint64_t len = min(min(reader().bytes_buffered(), modified_window_size_ - n_outstanding_ -
    // msg.sequence_length()), TCPConfig::MAX_PAYLOAD_SIZE);
    uint64_t MAX_len
      = min( modified_window_size_ - n_outstanding_ - msg.sequence_length(), TCPConfig::MAX_PAYLOAD_SIZE );
    ...
    cout << "    "
         << "SYN=" << msg.SYN << ", FIN=" << msg.FIN << ", RST=" << msg.RST << "; " << _Out( msg.payload ) << endl;
#endif
  ...
  for ( auto message : outstanding_messages_ ) {
    cout << "Message: SYN=" << message.SYN << " ,FIN=" << message.FIN << " ,RST=" << message.RST
         << " ,seqno=" << message.seqno.get_Raw_Value_() << "; " << _Out( message.payload ) << endl;
  }
  cout << "===============================" << endl;
#endif
}
