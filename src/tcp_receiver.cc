...
void TCPReceiver::receive( TCPSenderMessage message )
{
  ...
  cout << "receive message: SYN=" << message.SYN << ", FIN=" << message.FIN << ", RST=" << message.RST
       << ", seqno=" << message.seqno.get_Raw_Value_() << " , payload with length " << message.payload.size()
       << ": <" << _Out( message.payload ) << ">" << endl;
#endif
  // Your code here.
  if ( message.RST == true ) {
    RST_set = true;
...
#ifdef TCPRECEIVER_DEBUG
  cout << ">>>"
       << "Insert: first_index = " << first_index << ", payload = " << message.payload
       << ", finished = " << message.FIN << endl;
#endif
...
#endif
    message.ackno
      = Wrap32::wrap( reassembler_.get_Base_Position_() + SYN_set + reassembler_.writer().is_closed(), SYN );
  }
#ifdef TCPRECEIVER_DEBUG
  cout << "BS: available = " << reassembler_.writer().available_capacity()
       << ", RA: pending = " << reassembler_.bytes_pending() << endl;
#endif
