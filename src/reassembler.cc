...
void Reassembler::insert( uint64_t first_index, std::string data, bool is_last_substring )
{
#ifdef REASSEMBLE_DEBUG
    cout << "---------" << endl;
    cout << "Accepted: <" << _Out(data) << "> at position " << first_index << ", is_last = " << is_last_substring << endl;
#endif
    // Your code here.
    // (void)first_index;
    // (void)data;
    ...
    // list_.insert(insertpos, Node(data.size(), first_index, std::move(data), is_data_complete && is_last_substring));
    if(insertpos_default) {
        list_.push_back(Node(curdata_size, first_index, std::move(data), is_data_complete && is_last_substring));
    } else {
        list_.insert(insertpos, Node(curdata_size, first_index, std::move(data), is_data_complete && is_last_substring));
    }
    ...
    for(auto &node: list_) {
        ...
        #ifdef REASSEMBLE_DEBUG
            cout << "Pushed: <" << _Out(node.content) << ">, is_final = " << node.is_final << ", now base_position_ = " << base_position_ << endl;
        #endif
